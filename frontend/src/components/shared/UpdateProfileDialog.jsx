...
export default function UpdateProfileDialog({ open, setOpen }) {
  ...
  const [input, setInput] = useState({
    ...
    skills: user?.profile?.skills?.join(", ") || "", // assuming input is a string
    file: null,
    profilePhoto: null,
  });
  ...
  const submitHandler = async (e) => {
    ...
    if (input.profilePhoto) {
      formData.append("profilePhoto", input.profilePhoto); // This is for profile image
    }
    try {
      setLoading(true);
            ...
            <DialogFooter>
              {loading ? (
                <Button className="w-full my-4">
                  <Loader2Icon className="mr-2 h-4 w-4" /> Please Wait{" "}
                </Button>
              ) : (
                <Button
